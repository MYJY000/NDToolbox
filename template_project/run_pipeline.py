...
from ndbox.processor import run_processor, TRAIN_MASK, TEST_MASK
...
def parse_options(root_path):
    ...
    temp_path = path.join(root_path, 'temp_files')
...
def run_pipeline(root_path):
    ...
    for exp_name, exp_opt in opt.get('experiment', {}).items():
        ...
        copy_dataset = processor_pipeline(dataset, processor_opt, temp_path)
            ...
            if train_data_name is not None:
                ...
                if len(split_col) == 0:
                    ...
                    for idx, col in enumerate(split_col):
                        ...
                        model = train_pipeline(train_x[mask == TRAIN_MASK], train_y[mask == TRAIN_MASK],
                                               save_path, model_opt, logger)
                        model_list.append(model)
        ...
        for dataset_name, image_path in copy_dataset.items():
            dataset[dataset_name].restore_image(image_path)
    ...
    for idx, model in enumerate(model_list):
        if mask is not None:
            result = model.validation(
                test_x[mask[:, idx] == TEST_MASK], test_y[mask[:, idx] == TEST_MASK],
                metric_list, label['col'], label['name'][idx]
            )
        else:
...
def processor_pipeline(dataset, processor_opt, temp_path):
    copy_dataset = {}
    for dataset_name, dataset_opt in processor_opt.items():
        is_copy = dataset_opt.get('is_copy', True)
        image_path = path.join(temp_path, f'{dataset_name}_image_{int(time.time())}')
        nwb_dataset = dataset[dataset_name]
        if is_copy:
            os.makedirs(image_path)
            copy_dataset[dataset_name] = image_path
            nwb_dataset.save_image(image_path)
